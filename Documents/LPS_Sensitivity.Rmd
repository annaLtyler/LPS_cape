...
temp.decomp <- plot.decomp(temp, cols = ordered_cl)
...
par(mfrow = c(2,4))
for(i in 1:ncol(temp)){
    plot.with.model(temp[,i], temp.decomp$u[,2,drop=FALSE], main = colnames(temp)[i], xlab = colnames(temp)[i],
        ylab = "Temp PC1")
}
```
...
temp.pc2  <- temp.decomp$u[,2,drop=FALSE]
colnames(temp.pc2) <- "Temp_PC2"

par(mfrow = c(1,2))
plot.with.model(temp.min, temp.pc, main = "Temp PC1 vs. Tempeature Miminum")
plot.with.model(temp.max, temp.pc2, main = "Temp PC2 vs. Tempeature Maximum")
```
...
text(x = 50, y = 0.265, labels = "More LPS resistant", adj = 1)
arrows(x0 = 60, x1 = 90, y0 = 0.25, lwd = 3)
text(x = 60, y = 0.265, labels = "More LPS sensitive", adj = 0)
```
