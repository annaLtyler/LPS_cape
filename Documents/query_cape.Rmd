...
#load local functions again to mask distal functions
all.fun <- list.files(here("Code"), full.names = TRUE, pattern = ".R")
for(i in 1:length(all.fun)){source(all.fun[i])}
...
#plot_variant_influences_query(final_obj, geno_obj, p_or_q = 0.05, scale_coord = 1e6, coord_label = "Mbp")
#dev.off()

#plot deviation from additivity across the genome
pdf(here("Results", results_name, "Variant_Effects.pdf"), width = 10, height = 7)
plot_variant_effects_query(final_obj, geno_obj, p_or_q = 0.05, scale_coord = 1e6)
dev.off()
...
clin.effect <- clinical.effects.int(final_obj, query_geno, 
    clinical.benefit = c("l", rep("*", 5)),
