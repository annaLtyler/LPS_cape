...
if(is.na(results_dir)){
    results_dir <- here("Results", "cape_2ET_females")
}
```
...
```{r effect_dist, eval = FALSE}
has.vals <- which(sapply(clin.effects$details, function(x) is.finite(as.numeric(x[1,"expected.additive"]))))
all.exp <- lapply(has.vals, function(x) as.numeric(clin.effects$details[[x]][,"expected.additive"]))
all.act <- lapply(has.vals, function(x) as.numeric(clin.effects$details[[x]][,"actual"]))
all.diff <- lapply(1:length(all.exp), function(x) all.exp[x][[1]] - all.act[x][[1]])
diff.mean <- sapply(all.diff, mean)
mean.order <- order(diff.mean)
boxplot(all.diff[mean.order])

diff.mat <- Reduce("rbind", all.diff)
colnames(diff.mat) <- colnames(data_obj$pheno)
rownames(diff.mat) <- names(clin.effects$details)[has.vals]
pdf("~/Desktop/ints.pdf", width = 7, height = 11)
pheatmap(diff.mat)
dev.off()
```
