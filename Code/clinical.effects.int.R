...
clinical.effects.int <- function(data.obj, geno.obj, clinical.benefit, p.or.q = 0.05, 
	...
	if(geno.coding == "Dominant"){
		geno[which(geno.obj >= 0.5)] <- 1
		}
	if(geno.coding == "Recessive"){
		geno[which(geno.obj <= 0.5)] <- 0
		}

	var.inf <- write_variant_influences(data.obj, p.or.q, include_main_effects = FALSE, write_file = FALSE)	
	...
	get.genotype <- function(marker.name){
		split.marker <- strsplit(marker.name, "_")
		just.marker <- split.marker[[1]][1]
		just.allele <- split.marker[[1]][2]
		marker.locale <- which(dimnames(geno)[[3]] == just.marker)
		allele.locale <- which(dimnames(geno)[[2]] == just.allele)

		if(length(marker.locale) == 1){
			return(geno[,allele.locale,marker.locale])
			}else{
			marker.locale <- which(data.obj$p_covar == marker.name)	
			if(length(marker.locale) == 1){
				return(data.obj$p_covar_table[,marker.locale])
				}
			}
		}	
	...
	get.pheno.vals <- function(marker1, marker2, phenotype, test.plot = FALSE){		
		...
		marker1.geno <- round(marker1.geno*100)/100

		marker2.geno <- get.genotype(marker2)
		if(is.null(marker2.geno)){return(rep(NA, 6))}
		marker2.geno <- round(marker2.geno*100)/100
