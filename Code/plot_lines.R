...
plot_lines <- function(phenoV, marker1_vals, marker2_vals = NULL, pheno_name, marker1_label, marker2_label, ymin = NULL, ymax = NULL, error_bars){
	...
	if(is.null(marker2_vals)){
		...
		axes = FALSE, ylab = "", xlab = marker1_label, ylab = pheno_name, main = marker2_label,
		lwd = 3)
		axis(1, at = 1:length(pheno_means), labels = geno_bins)
		axis(2)
	...
	if(!is.null(marker2_vals)){
		...
		interaction.plot(marker1_vals[not_na], marker2_vals[not_na], phenoV[not_na], 
			xlab = marker1_label, trace.label = marker2_label, ylab = pheno_name, lwd = 3, 
			main = marker2_label, axes = FALSE, ylim = ylim, 
			fixed = TRUE)

		plot_lim <- par("usr")
		min_area = plot_lim[1]*1.17; max_area = plot_lim[2]*0.85
		axis(1, at = c(seq(min_area, max_area, (max_area-min_area)/(length(marker_geno_bins[[2]])-1))), labels = marker_geno_bins[[2]])
		axis(2)
