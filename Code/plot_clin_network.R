#based on plot_network, this function plots
#clinical effects from clinical.effects.int()


plot_network <- function(data_obj, clin.summary, marker_pairs = NULL, collapsed_net = TRUE, 
                        trait = NULL, trait_labels = NULL, color_scheme = c("DO/CC", "other"), 
                        main_lwd = 4, inter_lwd = 3, label_cex = 1.5, percent_bend = 15, 
                        chr_gap = 1, label_gap = 5, beneficial_col = "blue", 
                        harmful_col = "brown", show_alleles = TRUE){
  ...
  pos_col <- get_color(beneficial_col)[3]
  neg_col <- get_color(harmful_col)[3]
